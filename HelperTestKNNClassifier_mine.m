function result = HelperTestKNNClassifier_mine(trainedClassifier, features_test)
%HelperTestKNNClassifier Test a trained classifier
%
%  Input:
%      trainedClassifier : A trained KNN classifier
%      features_test     : A table containing the predictor (Pitch & MFCC)
...
[Fidx,Filenames] = findgroups(features_test.Filename);
result = table();

for idx = 1:length(Filenames)
    T = features_test(Fidx==idx,2:end);  % Rows that correspond to one file
    ...
    result_file.ActualSpeaker = T{1,15}{:};
